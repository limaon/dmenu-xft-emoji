...
static void keypress(XKeyEvent *ev)
{
	...
	switch (ksym) {
		...
		case XK_KP_Down:
			...
			break;
		case XK_Tab: /* Tab = navigate down */
			if (sel && sel->right && (sel = sel->right) == next) {
				curr = next;
				calcoffsets();
			}
			break;
		case XK_ISO_Left_Tab: /* Shift+Tab = navigate up */
			if (sel && sel->left && (sel = sel->left)->right == curr) {
				curr = prev;
				calcoffsets();
			}
...
static void paste(void)
{
	...
	if (XGetWindowProperty(
			dpy, win, utf8, 0, (sizeof text / 4) + 1, False, utf8, &da, &di, &dl, &dl, (unsigned char **)&p) ==
			Success &&
		p) {
...
static void setup(void)
{
	...
	xic = XCreateIC(
		xim, XNInputStyle, XIMPreeditNothing | XIMStatusNothing, XNClientWindow, win, XNFocusWindow, win, NULL);
